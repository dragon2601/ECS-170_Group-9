...
class EntropyBasedPlayer:
    ...
    def next_guess(self, current_state):
        ...
        next_underscore_position = current_state.find("_")
...
if __name__ == "__main__":
    ...
    word_length = st.number_input("Enter the length of the word: " )
    ...
    while remaining_lives > 0:
        st.write("Current state:", hangman.get_state())
        widget_counter += 1
        input_key = f"is_correct_input_{widget_counter}"
        guess = player.next_guess(hangman.get_state())
        if guess is None:
            print("AI is out of guesses!")
            break
        st.write("AI guesses:", guess)
        is_correct = st.text_input("Is the guess correct? (y/n): ", key=input_key).lower()
        if is_correct == 'y':
            positions = []
        ...
        if hangman.state and "_" not in hangman.get_state():
          st.write("AI won!")
          st.write("The word is:", hangman.get_state())
          break
    else:
        st.write("AI lost!")
